...
def qs_partition(li, left, right):
    ...
    divide_num = li[random.randint(left, right)]  # 设置“分割数”，这里可以优化一下
    while left < right:
...
def merge(head, low, mid, high):  # 实现归并,使得两个有序的序列归并成一个同序的序列
    p1 = low
    p2 = mid + 1
    temp = []
    ...
    while p2 <= high:
        temp.append(head[p2])
        p2 += 1
    head[low:high + 1] = temp


def merge_sort(head, low, high):
    if low < high:
        mid = (low + high) // 2
...
list1 = [random.randint(0, 114514) for num in range(100_000)]
